...
def modeling2(obstacles,original_data,distance_ratio):
    # mo
    n = len(original_data)
    rl=0.1
    ru = 1
    ...
    opti.solver('ipopt')
    sol = opti.solve()
    Q = np.array(sol.value(Q))
    
    
    displace = np.zeros((n,3))
    displace[1:,0:2] = (Q[1:,:]-Q[0:-1,:])*distance_ratio
...
def BubbleGeneration(sst,obstacles,rl,ru):
    ...
    for i in range(1,len(nearest_pts)-1):
        ...
        if r<rl:
            ...
            r = rl
            new_nearest = ces_obstacle.get_neareset_points([b])
...
def process(sst_data,obstacles,E,A,Iz,rho,distance_ratio=0.9,total_iters=500,dt_per_it=0.2,dt=0.0005,obstacle_force_ratio=500):
    ...
    sst_data_cp = cp.array(positions)
...
def process1(sst_data,obstacles,E,A,Iz,rho,distance_ratio=0.9,total_iters=500,dt_per_it=0.2,dt=0.0005,obstacle_force_ratio=500):
    ...
    current_positions = modeling1(obstacles,sst_data,distance_ratio)
    
    original_displacement = np.zeros((node_count,3),dtype=cp.float32)
    original_displacement[1:,] = (current_positions[1:,:]-current_positions[0:-1,:])*distance_ratio
    original_displacement[1:,] = np.cumsum(original_displacement[1:,],axis=0)
    # original_positions[1:,0:2] = current_positions[0:-1,0:2] + (current_positions[1:,0:2]-current_positions[0:-1,0:2])*distance_ratio

    
    theta0 = sst_data[0,2]-np.arctan2(current_positions[1,1]-current_positions[0,1],current_positions[1,0]-current_positions[0,0])
    if theta0>np.pi:
        theta0-=2*np.pi
    
    node_list = [Node(i,pos[0:2]) for (i,pos) in enumerate(current_positions)]
    element_list = [Element(i,node_list[i],node_list[i+1],E[i],A,Iz,rho) for i in range(element_count)]
    ...
    # F_xe=[internal_force,0,0,-internal_force,0,0]
    # for i in range(node_count-1):
    #     F_x[3*i:3*i+6] =  F_x[3*i:3*i+6]+np.matmul(element_list[i].T,F_xe)
    ...
    K_del = np.delete(K_del,constraint_index,axis=1)
    F_d_del = np.delete(F_d,constraint_index)
    # F_x_del = np.delete(F_x,constraint_index)
    # print(M_del)
    ...
    C_cp = cp.array(C)
    F_d_cp = cp.array(F_d_del,dtype=cp.float32)
    # F_x_cp = cp.array(F_x_del,dtype=cp.float32)
    ...
    positions_cp = cp.array(current_positions-original_displacement)
    sst_data_cp = cp.array(current_positions)
    
    displacement =cp.array(original_displacement)
    # displacement = cp.zeros((node_count,3),dtype=cp.float32)
    displacement[0,2] = theta0
    # displacement[:,0:2] = sst_data_cp[:,0:2]-positions_cp[:,0:2]
    

    # displacement[1:,0:2] = positions_cp[0:-1,0:2] + (positions_cp[1:,0:2]-positions_cp[0:-1,0:2])*distance_ratio
    # displacement[:,2]=positions_cp[:2,]
    ...
    print('integration time: ',integrator_time_elasped)
    print('obstacle force time: ',obs_time_elasped)
    return current_positions-original_displacement,displacement_history
...
def test():
    ...
    positions,displacement_history = process1(sst_state,obstacles,E,A,Iz,rho,distance_ratio=distance_ratio,total_iters=total_iters,dt_per_it=dt_per_it,dt=dt,obstacle_force_ratio=obstacle_force_ratio)
    time_end = time.time()  
...
def compare_fx():
    ...
    A=2*np.pi*r*r
    Iz =10*np.pi*r*r*r*r
    E=1e8*np.linspace(1,1,len(sst_state)-1)
    # E[0:3] = E[0]*10
    rho=20000
    total_iters=5
    dt_per_it=0.4
    ...
    positions1,displacement_history1 = process1(sst_state,obstacles,E,A,Iz,rho,distance_ratio=distance_ratio,total_iters=total_iters,dt_per_it=dt_per_it,dt=dt,obstacle_force_ratio=obstacle_force_ratio)
    time_end = time.time()  
    print('process time:',time_end-time_start)  
    ...
    ax.plot(displacement_history[-1][:,0]+positions[:,0],displacement_history[-1][:,1]+positions[:,1],label='Without Fx')
    ax.plot(displacement_history1[-1][:,0]+positions1[:,0],displacement_history1[-1][:,1]+positions1[:,1],label='With Fx')
    # for i,d in enumerate(displacement_history):
    #     if (i+1)%5==0:            
    #         ax.plot(d[:,0]+positions[:,0],d[:,1]+positions[:,1],label='{}'.format(i+1))
    #         # ax.plot(d[0:3,0]+positions[0:3,0],d[0:3,1]+positions[0:3,1],label='{}'.format(i+1))
