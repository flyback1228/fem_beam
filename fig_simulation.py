...
def test_fx():
    ...
    ax.plot(positions[:,0]+displacement_history[0][:,0],positions[:,1]+displacement_history[0][:,1],label='{}'.format(0))
    ...
    for i,d in enumerate(displacement_history):
        if (i+1)%10==0:            
            ...
            result_file = os.path.join(script_dir, 'result/simulation/it_{}.txt'.format(i+1))
            result.to_file(result_file)
            
    ax.arrow(0,0,np.cos(sst_state[0,2]),np.sin(sst_state[0,2]),width=0.1,label='initial direction')

    ces = ca.DM(displacement_history[0][:,0:2]+positions[:,0:2])    
    ces_dm = ca.DM(ces)
    ces_file = os.path.join(script_dir, 'result/simulation/ces.txt'.format(distance_ratio))
    ces_dm.to_file(ces_file)
